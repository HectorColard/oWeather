...
import { cityCountyFromDatas, cityLatitudeFromDatas, cityLongitudeFromDatas, cityNameFromDatas, cityTimezoneFromDatas, SUBMIT_POSITION_VALUE } from '../actions/position';
    ...
    case SUBMIT_POSITION_VALUE:
      axios.get(`https://geocoding-api.open-meteo.com/v1/search?name=${store.getState().position.citySearch}`)
        .then((response) => {
          ...
          store.dispatch(cityCountyFromDatas(response.data.results[0].admin1));
        })
        .catch((error) => {
          console.log(error);
        })
